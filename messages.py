...
def msg_command(command, server):
    ...
    if len(command.args) > 1:
        message = "Message from %s: %s" % (
            command.event.name, " ".join(command.args[1:]))
        ...
        if server.user_data.get_user(receipent):
            ...
            server.persistent_data.append(
                "messages.%s.inbox" % receipent, message)
...
def cm_command(command, server):
    ...
    if server.persistent_data.get("messages.%s.inbox" % user):
        ...
        if len(messages) > 0:
            messages_header_event = irc.Irc_event(
                "PRIVMSG", user, "Your current inbox:")
            server.send_event(messages_header_event)

            index = 0
            for message in messages:
                message_event = irc.Irc_event(
                    "PRIVMSG", user, "%d. %s" % (index, message))
                server.send_event(message_event)
                index += 1
                ...
                if server.persistent_data.get("messages.%s.autoremove" % user):
                    server.persistent_data.remove(
                        "messages.%s.inbox" % user, message)

            server.persistent_data.set("messages.%s.unseen" % user, False)
            return

    # Catch both cases for no messages
    no_messages_event = irc.Irc_event(
        "PRIVMSG", user, "You don't have any messages.")
                ...
                if not arg == "":
                    try:
                        ...
                        number_error_event = irc.Irc_event(
                            "PRIVMSG", user, "\"%s\" is no valid number." % arg)
    ...
    if len(command.args) > 0:
        if command.args[0] == "on":
            ...
            invalid_option_event = irc.Irc_event(
                "PRIVMSG", user, "\"%s\" is not valid option." % command.args[0])
